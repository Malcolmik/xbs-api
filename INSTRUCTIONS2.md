# XBS API Implementation Instructions - Section 2.2: Plan Management

## Overview
You are implementing Section 2.2 (Plan Management) for XBS (Xoro Billing Service). Plans define subscription tiers with pricing, billing intervals, and feature limits.

**Project Path:** `C:\Users\User\Desktop\xbs-api`
**Prerequisites:** Sections 1.3 and 2.1 must be completed first.

---

## Step 1: Create src/services/planService.ts
...
## Step 2: Create src/routes/plans.routes.ts
...
## Step 3: Create Database Migration (if plans table doesn't exist)

If the plans table wasn't created in earlier migrations, create `database/migrations/017_create_plans_extended.sql`:
...
**Add route registration (after customers route):**
```typescript
...
curl -X POST http://localhost:3000/v1/plans \
  ...
  -d '{
    ...
    "prices": [
      {"currency": "NGN", "unit_amount": 500000},
  ...
  -d '{
    ...
    "prices": [{
      ...
      "tiers": [
        ...
        {"up_to": 1000, "unit_amount": 50},
        {"up_to": null, "unit_amount": 25}
      ]
    }]
  }'
```
...
### Calculate Price:
```bash
curl -X POST http://localhost:3000/v1/plans/{plan_id}/calculate \
  -H "Authorization: Bearer xbs_sk_test_YOUR_KEY" \
  -H "Content-Type: application/json" \
  -d '{"currency": "NGN", "quantity": 500}'
```

### Archive Plan:
```bash
curl -X POST http://localhost:3000/v1/plans/{plan_id}/archive \
  -H "Authorization: Bearer xbs_sk_test_YOUR_KEY"
```

### Clone Plan:
```bash
curl -X POST http://localhost:3000/v1/plans/{plan_id}/clone \
  -H "Authorization: Bearer xbs_sk_test_YOUR_KEY" \
  -H "Content-Type: application/json" \
  -d '{"name": "Pro Plan"}'
```
...
| GET | /v1/plans/external/:external_id | Get by external ID |
| PATCH | /v1/plans/:id | Update plan |
| POST | /v1/plans/:id/archive | Archive plan |
| POST | /v1/plans/:id/unarchive | Restore plan |
| POST | /v1/plans/:id/clone | Clone plan |
| POST | /v1/plans/:id/calculate | Calculate price |
...
| tiered | Graduated pricing (each tier applies to units in range) |
| volume | Single tier applies to all units based on total quantity |

### Supported Currencies
NGN, USD, GBP, EUR, KES, GHS, ZAR, XOF, XAF, EGP, TZS
...
- **Section 2.2**: Plan management with multi-currency pricing, tiered pricing models, archiving, and cloning

Next section: **2.3 Subscription Management** - Creating and managing customer subscriptions to plans
